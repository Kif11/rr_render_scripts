...
{
    return previewFilename_Base_Python(IDnr)+"p%1_t.jpg";
}
QString  _JobSend::previewFilenameRGB_Python(const int &IDnr) const
{
    return previewFilename_Base_Python(IDnr)+"p%1.jpg";
}
QString  _JobSend::previewFilenameA_Python(const int &IDnr) const
{
    return previewFilename_Base_Python(IDnr)+"p%1_a.jpg";
}
#endif //DEF_Python
...
void _JobSend::fromMinInfo(const rrJ::_JobMinInfo * jobMinInfo,const bool &fillQuestionMark, const bool &clearOther)
{
    if (clearOther) {
        rrClearBasics();
        rrClearSubmitter();
